...
export const UnaiTechSection: React.FC<UnaiTechSectionProps> = ({ onOpenAdvisorWithPrompt }) => {
  ...
  return (
    <section
      id="unai-tech"
      className="relative w-full pt-[clamp(3.5rem,8vh,6rem)] pb-[clamp(3.5rem,8vh,6rem)] bg-[#0a1226] text-white overflow-hidden border-b border-cyan-500/15"
      aria-label="UNAI TECH Overview"
    >
      {/* Background glow effects */}
      <div className="absolute top-0 right-1/4 w-[600px] h-[400px] bg-cyan-500/15 blur-3xl pointer-events-none rounded-full" />
      <div className="absolute bottom-0 left-1/4 w-[600px] h-[400px] bg-blue-600/15 blur-3xl pointer-events-none rounded-full" />

      <div className="relative z-10 mx-auto max-w-[112rem] px-[max(1.5rem,7vw)]">
        
        {/* Section 1: UNAI TECH Core Overview */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center pb-14 border-b border-cyan-500/15">
          
          <div className="lg:col-span-6 flex flex-col gap-5">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0e1a38] text-cyan-300 text-xs font-black tracking-widest uppercase border border-cyan-500/40 w-fit shadow-xs">
              <Cpu className="w-3.5 h-3.5 text-cyan-300" />
              {UNAI_TECH_DATA.badge}
            </div>

            <div>
              <p className="eyebrow text-xs sm:text-sm text-cyan-300 font-extrabold tracking-widest uppercase mb-1">
            ...
            <div className="p-5 rounded-2xl bg-[#0e1a38]/80 border-2 border-cyan-500/40 shadow-xl backdrop-blur-xs">
              <span className="eyebrow text-[10px] text-cyan-300 tracking-widest font-black block mb-1">
                CORE PHILOSOPHY
              </span>
          ...
          <div className="lg:col-span-6">
            <h3 className="eyebrow text-xs font-black text-cyan-300/80 tracking-widest uppercase mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {UNAI_TECH_DATA.operatingAreas.map((area, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-xl bg-[#0e1a38] border border-cyan-500/20 hover:border-cyan-400 hover:bg-[#11224d] transition-all flex items-center gap-3.5 shadow-sm"
                >
                  <div className="w-9 h-9 rounded-lg bg-cyan-500/15 text-cyan-300 flex items-center justify-center shrink-0">
        ...
        <div className="pt-14">
          ...
          <div className="my-8 p-6 sm:p-8 rounded-2xl bg-[#0e1a38]/80 border border-cyan-500/25 shadow-2xl backdrop-blur-xs">
            <p className="eyebrow text-xs text-cyan-300 tracking-widest font-black uppercase text-center mb-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
              {WHAT_WE_ARE_BUILDING.ecosystemPipeline.map((node, idx) => (
                <React.Fragment key={idx}>
                  <div className="px-4 py-2.5 rounded-xl bg-[#0a1226] border border-cyan-400/50 text-white text-xs sm:text-sm font-black tracking-wider flex items-center gap-2 shadow-sm hover:border-cyan-300 hover:text-cyan-200 transition-colors">
                    <span className="w-2 h-2 rounded-full bg-[#38bdf8] shadow-[0_0_6px_#38bdf8]" />
                    <span>{node}</span>
                  </div>
                  {idx < WHAT_WE_ARE_BUILDING.ecosystemPipeline.length - 1 && (
                    <ArrowRight className="w-4 h-4 text-cyan-400 shrink-0 hidden sm:inline-block" />
                  )}
                </React.Fragment>
              ))}
            </div>
